#!/usr/bin/env bash
# bootstrap-macos.sh -- Install the Rust stable toolchain for FileForge Workbench
#
# Usage:
#   bash bootstrap/bootstrap-macos.sh
#   bash bootstrap/bootstrap-macos.sh --toolchain beta
#
...
LOG_FILE="${LOGS_DIR}/bootstrap-macos-${TIMESTAMP}.log"
...
log "=== FileForge Workbench -- Rust Bootstrap (macOS) ==="
log "CARGO_HOME:  ${CARGO_HOME}"
log "RUSTUP_HOME: ${RUSTUP_HOME}"
log "Toolchain:   ${TOOLCHAIN}"

if ! xcode-select -p >/dev/null 2>&1; then
    log "WARNING: Xcode Command Line Tools not detected." "WARN"
    log "  Pure-Rust builds do not require them, but some crates with C" "WARN"
    log "  dependencies may fail.  To install, run:" "WARN"
    log "    xcode-select --install" "WARN"
fi
...
if [ -x "${CARGO_BIN}/rustc" ]; then
    ...
    if command -v curl >/dev/null 2>&1; then
        ...
        log "ERROR: curl is required on macOS but was not found." "ERROR"
        exit 1
    fi
