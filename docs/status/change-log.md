...
- **Affects**: `crates/ff-theme/src/defaults.rs` (dark/light ui + tab_bar; legacy blue-on-black uses), `crates/ff-desktop/src/shell/render.rs` (`render_title_line` non-Legacy background), `crates/ff-desktop/src/shell/render_chrome.rs` (`render_tab_bar` reads `tab_bar.*`). Revises theme-and-appearance requirements (new criteria) + Req 13 note for Legacy legibility.
- **Status**: PENDING GATE
- **Linked spec**: `docs/specs/theme-and-appearance/requirements.md`
