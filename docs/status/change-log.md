...
- **Status**: IN PROGRESS. Shared renderer + Settings/menus DONE (Req 1.8/2.1a/2.1b PASS, pushed a0b8e76/8866bd8). POM-via-shared-renderer (Req 2.1c) GATE EXTENDED (Phase pom-via-shared-renderer): investigation found the POM is not a thin wrapper (compiled BUILT_IN_OPTIONS, focus ring, exit line, tab-kind chrome, and unresolved default pom.toml command strings), so Req 2.1d-2.1h were ADDED to capture the parity obligations (keep tab kind + [POM] title + Title_Line; focus ring from loaded options; data-driven terminate X->END; command reconciliation; fastpath preserved). Design delta + tasks 22.4a-22.4f/22.5/22.6 + master Phase (pom-via-shared-renderer) + TCR rows authored. IMPLEMENTATION next (owner approved: do POM gate + implementation before Theme work). Addresses the owner's "Settings should look like the POM" reports.
