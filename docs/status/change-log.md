...
- **Status**: IN PROGRESS -- Slice A implemented (option b). Gate run + approved; ff-file-tree-backed NavModel explorer is the DEFAULT File Explorer content: browse (local dirs + Mainframe catalog DSNs from SQLite + POSIX/Native catalog dirs), open (files + datasets via DSN resolution), keyboard nav, right-click context menu with Rename/Delete/New File/New Folder -- all via VFS providers (no std::fs in the modern path). Legacy inline tree remains behind a "Use legacy File Explorer" fallback toggle (default off). REMAINING: retire legacy inline tree (26.7-26.9), Tab focus-transfer onto the modern node list (26.12), migrate inline-model tests (26.14), manual accessibility pass. Slice B (mainframe qualifier/dataset duality + PDS member browsing, ADR-002 D2) is separate.
