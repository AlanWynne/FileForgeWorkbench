...
- [x] 13. StorageProvider trait in ff-vfs (Req 9)
  - [x] 13.1 Define `StorageProvider` trait in `src/storage_provider.rs` -- `allocate`, `open`, `stat`, `rename`, `delete`, `list`, `reconcile`; declare `StorageCapability` enum
  - [x] 13.2 Implement capability advertisement: each provider declares which operations it supports
  - [x] 13.3 Implement `UnsupportedOperation` default returns for optional methods
  - [x] 13.4 Register `StorageProvider` with `ProviderRegistry` alongside `VfsProvider`
  - [x] 13.5 Write unit tests for trait object construction, capability advertisement, and unsupported-operation defaults
  - Covers: Requirement 9.1-9.5

- [x] 14. POSIX files as native objects (Req 10)
  - [x] 14.1 Implement `PosixNativeProvider` -- maps POSIX catalog entries to native filesystem paths; no SQLite BLOB storage
  - [x] 14.2 Implement read/write delegation to native OS file I/O
  - [x] 14.3 Implement directory listing via `std::fs::read_dir`
  - [x] 14.4 Implement stat returning native file metadata (size, timestamps, permissions)
  - [x] 14.5 Implement path-safety guard -- reject traversal outside catalog root
  - [x] 14.6 Write unit tests for all POSIX provider operations using `tempfile::TempDir`
  - Covers: Requirement 10.1-10.6

- [x] 15. VFS staged transaction protocol (Req 11)
  - [x] 15.1 Define `VfsTransaction` struct -- wraps a sequence of VFS operations with commit/rollback semantics
  - [x] 15.2 Implement two-phase commit: stage all operations, then publish atomically
  - [x] 15.3 Implement rollback: undo staged operations in reverse order on failure
  - [x] 15.4 Implement transaction journal: persist in-progress state for startup recovery
  - [x] 15.5 Write unit and integration tests for commit, rollback, and interrupted-transaction recovery
  - Covers: Requirement 11.1-11.5

- [x] 16. workspace.backup / restore / reconcile / diagnose (Req 12)
  - [x] 16.1 Implement `workspace.backup` VFS command -- capture all provider state, write manifest with checksums
  - [x] 16.2 Implement `workspace.restore` VFS command -- validate manifest, restore to original or remapped root
  - [x] 16.3 Implement `workspace.diagnose` VFS command -- report orphaned objects and dangling entries
