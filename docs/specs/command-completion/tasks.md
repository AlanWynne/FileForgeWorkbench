...
- [x] 1. Crate scaffolding and module structure
  - [x] 1.1 Create `crates/ff-completion/Cargo.toml` with dependencies (ff-command, ff-vfs, ff-config, ff-logging, thiserror, serde, tokio, proptest dev-dep)
  - [x] 1.2 Create `crates/ff-completion/src/lib.rs` with module declarations and public API re-exports
  - [x] 1.3 Create module files: `candidate.rs`, `context.rs`, `provider.rs`, `registry.rs`, `engine.rs`, `matching.rs`, `popup.rs`, `navigation.rs`, `config.rs`, `error.rs`
  - [x] 1.4 Add `ff-completion` to workspace `Cargo.toml` members list
  - Covers: Structural foundation for all requirements

- [x] 2. CompletionCandidate model
  - [x] 2.1 Define `CompletionCandidate` struct with fields: label (String), insertion_value (String), category (Option<String>), description (Option<String>), icon_ref (Option<String>), sort_priority (u32)
  - [x] 2.2 Define `CandidateKind` enum: CommandName, FilePath, Directory, Keyword, MacroName, LineCommand, Custom
  - [x] 2.3 Implement `Display`, `Debug`, `Clone`, `PartialEq`, `Eq` for `CompletionCandidate`
  - [x] 2.4 Implement `Ord` / `PartialOrd` based on sort_priority then label for default ordering
  - [x] 2.5 Write unit tests for candidate construction, ordering, and kind discrimination
...
- [x] 4. CompletionProvider trait and ProviderRegistry
  - [x] 4.1 Define `CompletionProvider` trait with method `fn provide_candidates(&self, context: &CompletionContext) -> Result<Vec<CompletionCandidate>, CompletionError>` and async variant
  - [x] 4.2 Define `ProviderRegistration` struct: provider (Box<dyn CompletionProvider>), target_commands (Vec<String> or wildcard), argument_patterns (Vec<String>)
  - [x] 4.3 Implement `ProviderRegistry` struct with thread-safe storage (`RwLock<Vec<ProviderRegistration>>`)
  - [x] 4.4 Implement `register_provider(registration: ProviderRegistration) -> Result<ProviderId, CompletionError>`
  - [x] 4.5 Implement `deregister_provider(id: ProviderId)` for plugin cleanup
  - [x] 4.6 Implement `find_providers(context: &CompletionContext) -> Vec<&dyn CompletionProvider>` returning applicable providers for a context
...
- [x] 8. Built-in provider — Command Name completion
  ...
  - [x] 8.4 Apply frequency weighting from CommandHistory (if available) to sort_priority
  - [x] 8.5 Set insertion behaviour: replace prefix with canonical uppercase command name, append trailing space
...
- [x] 9. Built-in provider — File Path completion
  ...
  - [x] 9.6 Ensure VFS query is non-blocking (async/await, no UI thread blocking)
...
- [x] 13. Popup positioning model
  - [x] 13.1 Define `PopupAnchor` struct: x (f32), y (f32), field_height (f32)
  - [x] 13.2 Define `PopupBounds` struct: x (f32), y (f32), width (f32), height (f32)
  - [x] 13.3 Define `ViewportBounds` struct representing the application window dimensions
  ...
  - [x] 13.11 Ensure popup never overlaps command field text
  - [x] 13.12 Write unit tests for below placement, flip-above, best-fit, width bounds, and resize recomputation
...
- [x] 16. Dismiss behaviour logic
  - [x] 16.1 Implement dismiss on Escape key press
  - [x] 16.2 Implement dismiss on field focus loss
  - [x] 16.3 Implement dismiss when cursor retreats past anchor position (`cancel_at_start_pos` config)
  - [x] 16.4 Implement dismiss when filter produces zero matches (`auto_hide` config)
  - [x] 16.5 Implement dismiss on command submission (Enter to execute)
  - [x] 16.6 Implement dismiss-and-retrigger when context changes (different argument position)
  - [x] 16.7 Write unit tests for each dismiss condition and configuration toggles
...
- [x] 20. Fuzzy match highlight positions
  - [x] 20.1 Implement `highlight_positions(query: &str, candidate_label: &str) -> Vec<usize>` returning character indices to highlight
  - [x] 20.2 Integrate highlight data into `CompletionCandidate` display metadata when fuzzy mode is active
  - [x] 20.3 Write unit tests for highlight position correctness with various fuzzy matches
  - Covers: Requirement 6 (AC 6.3)

- [x] 21. Command registration for built-in providers
  - [x] 21.1 Register `CommandNameProvider` as a built-in provider on engine initialization
  - [x] 21.2 Register `FilePathProvider` as a built-in provider for commands expecting file path arguments
  - [x] 21.3 Register `KeywordProvider` as a built-in provider for commands with known keyword arguments
  - [x] 21.4 Register `MacroNameProvider` as a built-in provider for macro invocation contexts
  - [x] 21.5 Register `LineCommandProvider` as a built-in provider for prefix-area contexts
  - [x] 21.6 Verify all built-in providers use the same `CompletionProvider` trait as plugin providers
  - [x] 21.7 Write integration test verifying all built-in providers are registered and discoverable
  - Covers: Requirement 10 (AC 10.6)

- [x] 22. Error types
  - [x] 22.1 Define `CompletionError` enum with variants: ProviderFailed { provider_id, source }, NoProviders, ConfigInvalid { key, value, fallback }, VfsQueryFailed(source), SessionNotActive, InvalidContext
...
- [x] 24. Property-based tests
  - [x] 24.1 Write PBT: Prefix match correctness property
  - [x] 24.2 Write PBT: Fuzzy match subsequence property
  - [x] 24.3 Write PBT: Fuzzy scoring monotonicity property
  - [x] 24.4 Write PBT: Navigation wrap-around property
  - [x] 24.5 Write PBT: CompletionList filter idempotence property
  - [x] 24.6 Write PBT: Popup positioning within viewport property
  - [x] 24.7 Write PBT: Configuration clamping property
  - [x] 24.8 Write PBT: De-duplication invariant property
  - [x] 24.9 Write PBT: Insertion preserves trailing text property
  - Covers: All requirements (property-based validation)

- [x] 25. Integration tests
  - [x] 25.1 Write integration test: full command name completion flow (trigger → filter → accept → verify insertion)
  - [x] 25.2 Write integration test: argument completion with keyword provider
  - [x] 25.3 Write integration test: line command completion in prefix area
  - [x] 25.4 Write integration test: dynamic re-filter as user types additional characters
  - [x] 25.5 Write integration test: dismiss behaviours (escape, focus loss, empty matches)
  - [x] 25.6 Write integration test: plugin provider registration and candidate merging
  - [x] 25.7 Write integration test: config hot-reload updates engine behaviour
