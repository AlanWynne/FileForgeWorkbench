...
3. WHEN a Workspace is a Config View (the flat configuration-key browser opened by `CONFIG`, optionally namespace-filtered), THE Session_State SHALL persist it as `CustomWorkspace { workspace_kind = config, params = { namespace } }`, and on restore SHALL re-open the Config View with that namespace filter applied. (REVISED by CR-CH-025: the persisted `workspace_kind` is `config`, not `settings`; "Settings" now names only the menu. Realises configuration-system Requirement 20.7 and folds in the superseded CR-CH-011.)
