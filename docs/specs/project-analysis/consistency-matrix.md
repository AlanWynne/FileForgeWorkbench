...
| VSAM record ops (KSDS/ESDS/RRDS/LDS) | ADR-001 Req 5 names dedicated `ff-vsam-services`; today inside ff-dscatalog (`storage/`) | -- | No | Reinforces PA-SPLIT-008 (W2.1): governance already anticipates extracting VSAM from ff-dscatalog into ff-vsam-services |
