...
| menu-workspace-pattern raw-TOML stores | command-configurator (`commands.toml`) + ff-select (`.criteria.json`, PA-WATCH-015) + menu-workspace (`menus/`) | -- | Watch | PA-WATCH-019 (W3.4): a family of user-data TOML stores loaded via raw std::fs + own hot-reload, bypassing ff-config layers. Assess a shared loader/hot-reload helper OR confirm intentional. Confirm pattern at menu-workspace (W3.5). Not a VFS/FFW-ARCH-001 issue (local config files) |
| shell.mode external-execution security gate | shell-command (owns `shell.mode`) | command-configurator (Detached/Captured external commands inherit the gate) | Watch | PA-WATCH-018 (W3.4): confirm shell.mode actually gates configurator-launched external commands at shell-command (W3.8) |
