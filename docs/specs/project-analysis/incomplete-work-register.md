...
| PA-LOG-001 | LOGGING-GAP (ASCII, project-wide) | platform-core (`ff-core`) + project-wide | `.rs` sources contain non-ASCII in comments AND log-message string literals (em-dashes, box-drawing separators), violating rust-standards.md / documentation.md (Rust = plain ASCII only). ff-core: 322 matches incl. em-dashes in shutdown.rs:105 / lifecycle.rs:241/344 log strings (render in log file). | Schedule ONE project-wide code-mode cleanup pass replacing non-ASCII in `.rs` with ASCII (`--`, `===`), EXCLUDING genuine Unicode test/data literals; do not fix per-unit. Low severity. | later (dedicated) |
