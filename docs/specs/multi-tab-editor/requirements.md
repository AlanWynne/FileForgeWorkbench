...
7. WHEN `SWAP` is issued with NO argument AND NO split screen is active AND a Previous_Active_Tab exists (criterion 18.9), THE workbench SHALL activate the Previous_Active_Tab -- an Alt+Tab-style toggle between the two most-recently-active tabs -- WITHOUT reporting an error and WITHOUT opening the picker. (REVISED [CR-CH-031]: was "open the SWAP tab picker"; bare SWAP is now the quick previous-workspace toggle. The explicit picker remains available via `SWAP LIST`, criterion 18.3.)
...
9. THE workbench SHALL track a Previous_Active_Tab: the tab that was active immediately BEFORE the current Active_Tab. WHEN the Active_Tab changes (by any means -- `SWAP n`, click, `=`/menu navigation, open, close), THE prior Active_Tab identity SHALL become the Previous_Active_Tab. A bare `SWAP` (criterion 18.7) that toggles to the Previous_Active_Tab SHALL leave the just-left tab as the new Previous_Active_Tab, so repeated bare `SWAP` ping-pongs between the same two tabs. (This Previous_Active_Tab is a single most-recent-other-tab pointer; it is distinct from and does not require the full MRU_Stack of Requirement 7, which remains deferred.)
10. WHEN `SWAP` is issued with NO argument AND NO split screen is active AND NO Previous_Active_Tab exists (only one tab is open, or the previously active tab has since been closed and no other prior tab is known), THE workbench SHALL fall back to opening the SWAP tab picker (criterion 18.3) rather than reporting an error, preserving discoverability.
