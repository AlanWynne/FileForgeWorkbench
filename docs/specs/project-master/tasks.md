...
| `[~]` Phase DB | Unified Command Target + Command Configurator + Descriptor Persistence -- spec (DB.1-DB.7) done. Impl: DB.8 DONE (CommandTarget), DB.11 DONE (descriptor persistence, unblocks Task 14.5), DB.9 DONE (Command_Store + resolver + Command Configurator Context UI: COMMANDS/[COMMANDS], table, Add/Edit/Delete, variant editor, descriptor persistence -- command-configurator Task 4 + 6.1), DB.10 DONE (ff-shell external Detached/Captured execution), DB.4 DONE (menu-option + shortcut binding routed through Target_Resolution; inline `[options.target]`). Remaining: `MENU <name>` command + External/CustomWorkspace/Macro binding execution + prompt-confirm + placeholder expansion (external desktop adapter, command-configurator Task 3) |
