...
| `ff-desktop` | 🔴 | -- | Req 18.2 (CR-CH-019): built-ins are code-only, never materialised to themes/; a built-in stays a permanent read-only baseline |
...
| `ff-desktop` | 🔴 | -- | Req 19.1/19.2 (REVISED CR-CH-019): `themes/` dir created (possibly empty) on first launch; built-ins are NOT materialised -- themes/ holds only user themes |
| `ff-theme`/`ff-desktop` | 🔴 | -- | Req 19.2a (CR-CH-019): `list_all_themes` de-duplicated by name (built-in wins); no theme appears twice; a user file cannot shadow a built-in name |
...
| `ff-desktop` | 🔴 | -- | Req 20.5 (REVISED CR-CH-019 / B052): Save on a built-in redirects to Save As (never writes a built-in); Save As works after editing a token (button action not clobbered by token lost_focus) |
...
| `ff-desktop` | 🔴 | -- | Req 20.7/18.4 (REVISED CR-CH-019): reset a built-in re-selects the compiled palette (no file restore); user-theme reset restores `base` colours |
