...
| `ff-desktop` | ✅ | `shell/tests.rs::menu_bar_uses_kind_menu_bar_else_default` | workspace-kinds Req 4.1/4.2 (B.2): the rendered Menu_Bar is the active Kind's configured `menu_bar` (resolve_menu_bar_menu_for); `None` -> DEFAULT_MENU_BAR_NAME; detached bar uses its own Kind (active-tab via swap) |
| `ff-desktop` | ✅ | `shell/tests.rs::key_list_context_uses_kind_key_list_else_base` | workspace-kinds Req 4.3 (B.2): the key-map context is the active Kind's configured `key_list` if set (key_list_context_for_tab), else the base kind context; unknown key_list -> global map fallback via resolver precedence |
| `ff-desktop` | ✅ | `shell/tests.rs::{menu_bar_uses_kind_menu_bar_else_default, key_list_context_uses_kind_key_list_else_base}` | workspace-kinds Req 4.4/4.5 (B.2): a user Kind with unset menu_bar/key_list inherits its base's defaults (unwrap_or base default); built-in Kinds unchanged (behaviour-preserving) |
| `ff-desktop` | ✅ | `shell/tests.rs::new_editor_takes_kind_edit_profile_on_open` | workspace-kinds Req 5.1 (B.3): an editor Workspace's Edit_Profile is initialised from the Kind's profile ONCE at open (apply_kind_profile_to_active); a later per-tab toggle is not clobbered |
| `ff-desktop` | ✅ | `shell/tests.rs::{new_buffer_takes_kind_line_end_mode, line_end_from_name_maps_unicode_else_default}` | workspace-kinds Req 5.2 (B.3): a NEW/untitled buffer takes the Kind's line_end_mode default; a LOADED file keeps its detected mode (FileEditor arm sets only edit_profile) |
| `ff-desktop` | 🔲 | -- | workspace-kinds Req 5.3 (B.3): tab_size carried + round-trips but NOT applied to a per-tab size in B.3 -- DOCUMENTED deferral (no per-tab tab-size field; editor.tab_size stays global config). Applied when a per-tab tab-size exists / via B.4 dialog |
| `ff-desktop` | ✅ | `shell/tests.rs::new_editor_takes_kind_edit_profile_on_open` | workspace-kinds Req 5.4/5.5 (B.3): a Kind with a default profile opens with EditProfile::default() (behaviour-preserving for built-ins) |
| `ff-desktop` | ✅ | `kinds_editor_panel/tests.rs`, `shell/tests.rs::kinds_editor_new_kind_seeds_copy_modelled_on_base` | workspace-kinds Req 6.1/6.2 (B.4): Kinds Editor Context ([KINDS]) -- Kind selector + editable KindConfig form (title/menu_bar/key_list/modelled_on/profile) + "New Kind modelled on <base>" |
| `ff-desktop` | ✅ | `shell/tests.rs::kinds_editor_save_writes_file_and_reloads_registry` | workspace-kinds Req 6.3 (B.4): Save writes workspace-kinds/<name>.toml + reloads the registry live (built-in Save writes a user override) |
| `ff-desktop` | ✅ | `shell/tests.rs::open_kinds_editor_activates_kinds_editor_context`, `menu_workspace/defaults.rs::default_settings_toml_has_recovery_baseline_options` | workspace-kinds Req 6.4 (B.4): Kinds Editor reachable by the KINDS command AND a Settings entry (W -> Kinds; command parity) |
| `ff-desktop` | ✅ | `shell/tests.rs::full_shell_kinds_first_tab_focuses_first_interior` | workspace-kinds Req 6.5/6.6 (B.4): pure render -> action (shell applies); participates in the unified Tab-order model (WorkspaceContext InteriorFocus); KindsEditor is itself a built-in Kind |
| `ff-desktop` | ✅ | `shell/tests.rs::reset_bare_restores_builtin_kind_registry`, `shell/reset_bare.rs::archived_items_includes_workspace_kinds` / `archive_config_moves_workspace_kinds_dir` | workspace-kinds Req 7.1/7.2/7.3 (B.4): RESET BARE resets the Kind registry to compiled built-in defaults + archives the profile's workspace-kinds/ dir; built-in defaults always present after |
...
| `ff-desktop` | ✅ | `shell::tests::menu_bar_uses_kind_menu_bar_else_default` | menu-workspace Req 17.9 (CR-NR-080 Slice C): a Menu_Bar assigned to a workspace Kind (the Kind's effective `menu_bar` name, CR-NR-090) renders for instances of that kind via `resolve_menu_bar_menu_for` (used by both the app-level `render_menu_bar` and the split-region `render_region_menu_bar`, CR-CH-041); an unassigned kind falls back to `DEFAULT_MENU_BAR_NAME`. Delivered by the per-Kind `menu_bar` field + the resolver, mirroring the per-kind keymaps pattern (CR-CH-027) |
...
| `ff-session` | ✅ | `session_state.rs::menu_descriptor_round_trips_through_toml` | Req 21.4: Menu_Workspace persists as MenuWorkspace{name} (round-trip) |
| `ff-desktop` | ✅ | `shell/tests.rs::{restore_reopens_menu_descriptor_and_continues, restore_menu_descriptor_missing_file_opens_load_error_not_dropped, restore_pom_menu_descriptor_is_left_to_the_pom_guarantee}` | Req 21.4 (CR-CH-012 menu-restore slice): the restore loop RE-OPENS a persisted non-POM Menu descriptor via `open_menu_workspace_tab` (load-error state when `menus/<name>.toml` is absent, not dropped); the POM descriptor is left to the index-0 POM guarantee (Req 21.8), no duplicate |
...
### CR-NR-078 -- WorkspaceContext trait framework (phase 1 + WF.6 complete)
...
| `ff-desktop` | ✅ | `shell::tests::{full_shell_plugin_manager_first_tab_focuses_interior, full_shell_event_log_first_tab_focuses_interior, full_shell_macro_library_first_tab_focuses_interior, full_shell_command_configurator_first_tab_focuses_interior, full_shell_search_results_first_tab_focuses_interior}` (all pass unchanged after migration) | Req 1.4 (WF.6 8.1): Plugin Manager, Event Log, Macro Library, Command Configurator, Search Results each `impl WorkspaceContext` and dispatch via `render_workspace_context`; action-returning panels stash a `pending_*` the shell drains (`apply_macro_library_action`/`apply_configurator_action`/`apply_search_outcome`); Event Log applies its Clear-Log request in place via `services.notifications`; `ShellServices` gained read-only `command_store` |
...
| `ff-desktop` | ✅ | `tab_manager::tests::move_tab_to_group_moves_and_focuses_target`; `shell::tests::full_shell_move_tab_between_regions_moves_and_focuses` | layout-and-docking Req 14.6: drag a Tab_Header and drop onto another region moves the TabState to that group (by TabId), makes it active, focuses the target |
| `ff-desktop` | ✅ | `tab_manager::tests::move_tab_emptying_source_collapses_split`; `shell::tests::full_shell_move_tab_emptying_region_collapses` | layout-and-docking Req 14.7: a move that empties the source group collapses it via `remove_empty_groups` |
| `ff-desktop` | ✅ | `tab_manager::tests::move_tab_to_own_group_is_noop`; `shell::tests::full_shell_split_detach_still_detaches_not_splits` (detach gesture intact) | layout-and-docking Req 14.8: drag OUTSIDE the workbench still DETACHES; drop onto another region moves; drop onto own region is a no-op |
| `ff-desktop` | 🔲 | render: `render_split_region` Drop_Zone highlight (translucent accent fill + 3px border) on the non-source region under the pointer; drag-state driven (`full_shell_split_tab_drag_state_clears_on_release`) | layout-and-docking Req 14.9: a Drop_Zone highlight marks the region under the pointer during a tab drag. MANUAL: the pixel-exact highlight + live pointer-drag hover cannot be driven headlessly (egui_kittest has no cross-region pointer drag) |
| `ff-desktop` | ✅ | `tab_manager::tests::{layout_snapshot_captures_shape_and_focus, layout_snapshot_is_none_when_unsplit}`; `session_manager::tests::split_layout_round_trips_through_session` | layout-and-docking Req 14.10: on exit the split arrangement (structural descriptor: tree shape + per-node direction/proportion + per-leaf tab COUNT + focused-leaf index) is serialised into `SessionState.layout`. NOTE: identity-free (not tab ids) -- restore does not preserve TabIds (design.md refinement) |
| `ff-desktop` | ✅ | `tab_manager::tests::layout_snapshot_restore_round_trips_shape`; `session_manager::tests::split_layout_round_trips_through_session` | layout-and-docking Req 14.11: on launch a persisted layout rebuilds the split tree shape (store tabs distributed across leaves by count) and restores the focused leaf |
| `ff-desktop` | ✅ | `session_manager::tests::{unsplit_workbench_persists_no_layout, older_session_without_layout_field_loads}` | layout-and-docking Req 14.12: no persisted layout (older/unsplit session) opens UNSPLIT exactly as 2a/2b; backward-compatible (`#[serde(default)]`, no schema bump); older session.toml loads without error |
| `ff-desktop` | ✅ | `tab_manager::tests::{restore_layout_reconciles_extra_store_tabs, restore_layout_noop_with_single_tab}` | layout-and-docking Req 14.13: restore is self-consistent -- leftover store tabs (count mismatch) placed in the first leaf, empty leaves collapse via `sync_layout`; no tab lost, no dangling id |
| `ff-desktop` | ✅ | `shell::tests::{with_workspace_context_saves_and_restores_primary_context, detached_command_acts_on_its_tab_not_the_primary}` | layout-and-docking Req 14.14: the Focus_Context concept has two save/install/restore faces -- `with_workspace_context` (detached: flat-active + per-window command buffers) and `set_render_focus_leaf` (in-window: tree focus); the detached swap is defined in ONE place. (Scoped refactor-toward-unification, not a `FloatingTab` teardown) |
| `ff-desktop` | ✅ | `shell::tests::{full_shell_dock_command_redocks_tab_at_origin, full_shell_detach_sets_floating_and_records_floating_tab, full_shell_detach_rejected_at_16_window_limit, detached_function_key_return_acts_on_its_tab, full_shell_dock_unsplit_still_restores_flat_origin}` | layout-and-docking Req 14.15: existing Detached_Workspace behaviour (Req 18: detach/DOCK/16-limit/per-window command line/F-keys) intact after the fold-in (no regression) |
| `ff-desktop` | ✅ | `shell::tests::full_shell_dock_into_split_reattaches_to_leaf`; `tab_manager` `dock_tab_into_leaf` | layout-and-docking Req 14.16: `DOCK` re-attaches a detached tab into a tree leaf when split (the leaf that still owns it, else the focused leaf), not a flat origin index; unsplit re-dock unchanged |
| `ff-desktop` | ✅ | ALL 2c sub-slices covered: 2c.1 nesting (`tab_manager::tests::*` + `ff-layout split_leaf*` + `full_shell_split*`/`full_shell_second_split_nests`); 2c.2 move (`move_tab_to_group*` + `full_shell_move_tab_*`); 2c.3 persistence (`layout_snapshot*`/`restore_layout*` + `session_manager::tests::split_layout_round_trips*`); 2c.4 fold-in (`full_shell_dock_into_split_reattaches_to_leaf` + Req 18 no-regression suite). Justified-MANUAL: pixel drag GESTURE, Drop_Zone highlight pixels, OS detached-window chrome | layout-and-docking Req 14.17: all 2c model ops unit-tested + full-shell egui_kittest for rendered behaviour; drag GESTURE pixels + OS detached-window chrome are justified-MANUAL |

### Phase (window-split-2d) -- Per-region command lines for split Tab_Groups (CR-NR-094, B046 Slice 2d)

> Each in-window split region gets its own `Command ===>` line + `WorkspaceCommandContext`,
> dispatched against that region's tab via the existing `with_workspace_context` path; the top-level
> field is suppressed while split. Per-region state is transient (not persisted).

| Crate | Status | Test files | Notes |
|-------|--------|-----------|-------|
| `ff-desktop` | ✅ | `shell/tests.rs::{full_shell_region_command_field_is_a_stable_tab_stop, full_shell_split_renders_two_regions_and_command_acts_on_focused}` | layout-and-docking Req 15.1: while split, each region renders its own `Command ===>` field (Region_Command_Line) within the region |
| `ff-desktop` | ✅ | `shell/tests.rs::{full_shell_region_command_acts_on_its_own_region, full_shell_region_command_from_non_focused_region_acts_and_focuses}` | layout-and-docking Req 15.2: a command submitted in a region's field dispatches against THAT region's active tab (via `with_workspace_context`), regardless of keyboard focus; never another region's tab |
| `ff-desktop` | ✅ | `shell/tests.rs::full_shell_region_command_text_and_status_are_isolated` | layout-and-docking Req 15.3: each region has its OWN command text / SCROLL / status; text + status are isolated per region |
| `ff-desktop` | ✅ | `shell/tests.rs::{full_shell_region_command_from_non_focused_region_acts_and_focuses, full_shell_region_command_acts_on_its_own_region}` | layout-and-docking Req 15.4: submitting a region command focuses that region (Focused_Group), consistent with Req 14.5/14.8 |
| `ff-desktop` | ✅ | `shell/tests.rs::{region_cmd_ctx_inserted_per_leaf_on_split, region_cmd_ctx_cleared_on_unsplit, region_cmd_ctx_text_does_not_travel_with_moved_tab}` | layout-and-docking Req 15.5: per-leaf Region_Context lifecycle -- fresh on SPLIT, dropped on collapse/merge, not carried by a moved tab, cleared on unsplit; no leak to an unrelated leaf |
| `ff-desktop` | ✅ | `shell/tests.rs::full_shell_split_suppresses_top_level_command_field_unsplit_restores` | layout-and-docking Req 15.6: UNSPLIT renders the single top-level `Command ===>` field byte-identically; per-region fields apply only while split (top-level suppressed while split -- design decision) |
| `ff-desktop` | ✅ | `shell/tests.rs::full_shell_region_command_field_is_a_stable_tab_stop` | layout-and-docking Req 15.7: Region_Command_Line participates in Tab-order/Boundary_Policy with a stable salted `egui::Id`; a region's command field is a real, focusable Tab stop (no phantom id, B056 class) |
| `ff-desktop` | ✅ | `shell/tests.rs::full_shell_region_command_state_is_transient` | layout-and-docking Req 15.8: per-region command state NOT persisted across restart (transient, like detached `cmd_ctx`); split-structure persistence (Req 14.10-14.13) unchanged |
| `ff-desktop` | ✅ | `shell/tests.rs` per-region suite (rows 15.1-15.8) | layout-and-docking Req 15.9: all per-region command-line behaviour covered by unit + full-shell egui_kittest; pixel-exact field placement is the only justified-MANUAL exception |
...
> Reframes ownership: a Workspace INSTANCE owns its chrome (title/menu bar/keylist/command line);
> a REGION is geometry/placement only; per-instance chrome (incl. the Kind menu bar) renders
> IN-REGION; placement (`Detached | Docked{position,size}`) is DERIVED from the layout snapshot, not
> double-stored on the Workspace_Descriptor; core has one tab system. Behaviour-preserving when
> unsplit. All rows NOT COVERED until implementation (gate pending owner approval).

| Crate | Status | Test files | Notes |
|-------|--------|-----------|-------|
| `ff-desktop` | ✅ | `tab_manager::tests::docked_leaf_of_*`; `shell::tests::placement_of_*` | layout-and-docking Req 16.1: the Workspace instance owns its chrome (title/menu bar/keylist/command line/scroll); a Region contributes only geometry + id. Chrome resolved for the instance (`resolve_menu_bar_menu_for`, `kind_title`, per-region `WorkspaceCommandContext`); Region is the `TabGroupTree` leaf (geometry + id) |
| `ff-desktop` | ✅ | `shell::tests::{full_shell_split_suppresses_app_level_menu_bar_and_title_unsplit_restores, full_shell_split_region_menu_bar_is_live_and_focusable}` | layout-and-docking Req 16.2: full per-instance chrome (menu bar, Title_Line, command line) renders INSIDE the instance's placement, not as a shared app-level bar; while split, each region draws its placed instance's full chrome (`render_split_region` -> `render_region_menu_bar` + `render_title_line_into_ui` + `render_region_command_field`; app-level bar/title suppressed in `update`) |
| `ff-desktop` | ✅ | `shell::tests::full_shell_split_suppresses_app_level_menu_bar_and_title_unsplit_restores` (unsplit-restores arm) | layout-and-docking Req 16.3: UNSPLIT single-instance case is visually + behaviourally byte-identical to today (region is the whole window); app-level chrome restored on UNSPLIT. Proven behaviour-identical by the full existing suite passing unchanged |
| `ff-desktop` | ✅ | `shell::tests::full_shell_placement_tracks_split_region_then_detached` | layout-and-docking Req 16.4: the three placements (docked-full / split-region / detached) render the SAME per-instance chrome through the SAME path (`render_menu_bar_into_ui` / `render_title_line_into_ui` shared by app-level, split-region, and detached); only Placement differs |
| `ff-desktop` | ✅ | `tab_manager::tests::docked_leaf_of_resolves_owning_leaf_when_split`; `shell::tests::full_shell_placement_tracks_split_region_then_detached` | layout-and-docking Req 16.5: instance id (`TabId`) is stable, unique, and distinct from region id (`TabGroupId`); preserved across move-between-regions and detach/redock |
| `ff-desktop` | ✅ | `tab_manager::tests::docked_leaf_of_*`; `shell::tests::placement_of_*` | layout-and-docking Req 16.6: Placement is DERIVED (`WorkbenchShell::placement_of` from `floating_tabs` + `TabManager::docked_leaf_of`), NOT duplicated on the Workspace_Descriptor; no-snapshot = single docked root region; layout tree is the single source of truth for placement |
| `ff-desktop` | ✅ | `shell::tests::kind_config_has_no_core_tab_container_field` | layout-and-docking Req 16.7: core provides exactly ONE tab system (Region/`TabGroupTree`); structural guard proves no `KindConfig` tab-container field; a Kind's internal composition is private |
| `ff-desktop` | ✅ | `shell::tests::{full_shell_split_region_menu_bar_is_live_and_focusable, full_shell_region_command_field_is_a_stable_tab_stop}` | layout-and-docking Req 16.8: per-instance in-region chrome satisfies the single shell-level Boundary_Policy + workspace-conformance (per-region-salted `egui::Id` via `push_id(("region_menu_bar", leaf))` / `("region_command_field_input", leaf)`, live focusable, no phantom stop, no second focus ring) |
...
| Crate | Status | Test files | Notes |
|-------|--------|-----------|-------|
| `ff-desktop` | ✅ | `shell::tests::pom_settings_click_equals_typed_settings`, `pom_option_key_type_and_click_same_result` | menu-workspace Req 19.1: one Option_Dispatch_Path shared by click / key / Tab+Enter / typed; selection == `activate_menu_option` -> `handle_command(command)` |
| `ff-desktop` | ✅ | `shell::tests::pom_settings_click_equals_typed_settings`, `pom_option_key_type_and_click_same_result` | menu-workspace Req 19.2: POM and non-POM share the one current-menu resolver (`try_current_menu_option` no longer skips `is_home`); POM click == Settings click; POM key type == click |
| `ff-desktop` | ✅ | `shell::tests::pom_settings_click_equals_typed_settings` (click seam) + `activate_menu_option` shared helper | menu-workspace Req 19.3: click path calls `activate_menu_option(option.target, option.command)` (no click-only pre-branch); inline `[options.target]` resolved in-pipeline (Req 10.6 preserved) |
| `ff-desktop` | ✅ | full `menu` suite (184 tests) incl. `settings_t_chains_to_theme_editor`, `menu_command_returns_to_home_context` | menu-workspace Req 19.4: Menu Workspace is a dumb dispatcher (`activate_menu_option` does not decide kind/placement); `=` fastpath + `<menu> <key>` chaining preserved |
| `ff-desktop` | ✅ | `shell::tests::clicking_pom_settings_option_opens_settings_menu_in_place`, `dispatch_menu_target_pom_opens_home_context` | menu-workspace Req 19.5: command owns in-place-vs-new-tab; `open_named_menu` router removed, folded into `open_menu_by_name` (pom/settings in place, others new tab) |
| `ff-desktop` | ✅ | `menu_workspace::commands::tests::disabled_option_shows_not_available_message` + one-path activation | menu-workspace Req 19.6: disabled-option message via `find_option` on the one path for every affordance (POM + non-POM) |
| `ff-desktop` | ✅ | `shell::tests::unknown_token_is_unresolved_not_a_menu` | menu-workspace Req 19.7: unresolved-command error emitted on the one path for every affordance |
| `ff-desktop` | ✅ | full `menu` suite (184 passed, 0 failed) incl. B075 + workspace-conformance focus tests | menu-workspace Req 19.8: behaviour-preserving; POM `Settings` click opens Settings in place |
| `ff-desktop` | ✅ | `shell::tests::pom_settings_click_equals_typed_settings`, `pom_option_key_type_and_click_same_result` | command-framework Req 14.1: every affordance reduces to `handle_command` via `activate_menu_option`; no private per-affordance pre-branch (Selection_Equals_Command) |
| `ff-desktop` | ✅ | `shell::tests::pom_settings_click_equals_typed_settings` (same landing state for click vs typed) | command-framework Req 14.2: two affordances selecting the same command with the same argument produce identical observable results |
...
### Phase (cmdline-position) -- CR-NR-095 per-Kind command-line position (workspace-kinds Req 8)
