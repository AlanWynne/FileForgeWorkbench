...
### Phase (nav-stack) -- Per-tab Navigation_Stack (CR-CH-022, Req 14; reconciles Req 5)

> Owner: "Every Workspace should have a path store so END navigates back up the
> path. Stack per tab; transform in place; the only time a new tab is created is
> START." Replaces the three ad-hoc END mechanisms (pending_return_to_pom flag,
> namespace_filter END branch, opened_from_settings bool) with one per-tab
> Navigation_Stack of WorkspaceDescriptors. Navigation always transforms the
> current tab in place (pushing the old Context); END pops one level; empty-stack
> END closes the Workspace (exits if last, CR-CH-016). START is the only
...
> file-backed and user-editable. Adds a "Default Legacy" built-in + reset baseline, a themes/
> directory materialised from the built-ins on first launch, startup loading of the active theme
...
### Phase (pom-via-shared-renderer) -- POM rendered through the shared menu renderer (CR-CH-018, task 22.4)

> Final slice of CR-CH-018: render the POM (Home Context) through the shared
> `render_menu_workspace` against a `menus/pom.toml`-backed MenuWorkspaceState,
> keeping TabKind::PrimaryOptionMenu, the `[POM]` title, the POM Title_Line, the
> keyboard focus ring (now sourced from the loaded options), the terminate
> action (data-driven `X`->END option), and `=0`/`=1`/`=2` fastpath. Retires the
...
- [x] NAV.6 Retired the inline FileExplorerPanelState tree -- file_explorer_panel.rs + copy_move_dialog.rs deleted; modern explorer is the sole File Explorer; Req 15-23 features preserved (native OS dialog + attribute columns deferred, not core) (file-tree-panel Req 24.6)
- [x] NAV.7 Modern presentation via theme file_tree.* -- indent guides/glyphs/selection-focus-ring/colours done (manual accessibility pass tracked separately) (file-tree-panel Req 24.7)
...
- [x] NAV.9 Persistent shell Command ===> retained on the File Explorer Context; Tab focus-transfer re-pointed onto the modern ff-file-tree node list (first_row_id/next_row_id/nav_focused) (file-tree-panel Req 24.9)
- [x] NAV.10 Behaviour preservation: verify.ps1 FULL clean across all Slice A commits; inline-model tests removed with the legacy file, intent preserved by explorer_view/nav_model tests (file-tree-panel Req 24.10)
...
| Status | Count |
|--------|-------|
| `[x]` Phase (navigation-modernization) | Unified Navigation Model + File Explorer Modernization Slice A (CR-NR-060): file-tree-panel Requirement 24 -- DONE. Modern ff-file-tree/NavModel explorer is the SOLE File Explorer (NAV.1-12): browse/open(files+datasets)/keyboard/Tab-focus/context-menu(Open/Copy/Paste/CopyFullPath/Reveal/Rename/Delete/New)/multi-select/copy-as-text-tree/file-copy-paste, all via VFS providers (no std::fs). Legacy inline tree + Copy/Move dialog deleted; fallback toggle removed. DEFERRED (not core): native OS file-dialog + Req-18 attribute columns. Slice B (mainframe qualifier/dataset duality + PDS members, ADR-002 D2) separate |
...
- [x] WS2C.3 (2c.3 persistence): layout_snapshot/restore_layout on the tree; persist into
...
- [x] MDC.1 Requirements gate -- menu-workspace Req 19 (one Option-Selection path; dumb dispatcher), command-framework Req 14 (Selection-Equals-Command; command owns placement), design deltas in both design.md, tasks (menu-workspace Task 36, command-framework Task 34), TCR NOT COVERED rows, change-log CR-CH-043.
- [x] MDC.2 Failing full-shell tests first: POM `Settings` click == typed `SETTINGS` (in place); POM Option_Key type == click (same result). Covers menu-workspace Req 19.1/19.2; command-framework Req 14.1/14.2.
- [x] MDC.3 One current-menu Option_Key resolver (`try_current_menu_option` no longer skips `is_home`); POM and non-POM share it; `=` fastpath (`resolve_pom_option_key`) preserved for Navigation_Origin. Covers menu-workspace Req 19.2/19.4; command-framework Req 14.1.
- [x] MDC.4 Click path calls `activate_menu_option(option.target, option.command)` (drop the click-only target pre-branch); inline `[options.target]` resolved in-pipeline. Covers menu-workspace Req 19.3.
- [x] MDC.5 Fold `open_named_menu` name-switch into `open_menu_by_name` (`SETTINGS` in place, `POM` Home, user-menu new tab); `Menu` target arm + `try_menu_name_dispatch` call `open_menu_by_name`; `open_named_menu` removed. Covers menu-workspace Req 19.5; command-framework Req 14.3/14.4.
- [x] MDC.6 Close: disabled/unresolved messages on the one path; existing menu-workspace / B075 / workspace-conformance focus suite green (behaviour-preserving); verify.ps1 CLEAN FULL nextest; ffwb.exe rebuilt; TCR menu-workspace Req 19 + command-framework Req 14 PASS. Covers menu-workspace Req 19.6-19.8; command-framework Req 14.5/14.6.
...
- [x] CLP.1 Requirements gate -- workspace-kinds Req 8 (per-Kind command-line position), design delta, tasks (workspace-kinds Task 18), TCR NOT COVERED rows, change-log CR-NR-095.
- [x] CLP.2 Failing tests first: KindProfile TOML round-trip for `command_line_position`; pure `split_region_strip_rects` Bottom layout. Covers Req 8.1/8.2/8.4.
- [x] CLP.3 `CommandLinePosition` enum + `KindProfile.command_line_position` (default Top) + `command_line_position_for` resolution helper. Covers Req 8.1/8.2/8.6.
- [x] CLP.4 Apply in the three render paths: unsplit top/bottom panel, split-region position-aware strip rects, detached top/bottom panel. Covers Req 8.3/8.4/8.5.
- [x] CLP.5 Kinds Editor Top/Bottom control bound to the working profile (Save via the shared position-setting seam). Covers Req 8.7.
- [x] CLP.6 RESET BARE returns position to Top (test; no new code). Covers Req 8.8.
- [x] CLP.7 `COMMAND` primary command: `COMMAND TOP` / `COMMAND BOTTOM` set, bare `COMMAND` toggles the active instance's Kind position via one shared seam (persists to the Kind file); ordered so it does not collide with `COMMANDS`. Covers Req 8.9-8.13.
- [x] CLP.8 Close: full-shell egui_kittest (Bottom renders at foot, Tab-order unchanged; `COMMAND BOTTOM` moves the field); verify.ps1 CLEAN FULL nextest; ffwb.exe rebuilt; TCR Req 8 PASS. Covers Req 8.

| Status | Count |
|--------|-------|
| `[x]` Phase (cmdline-position) | CR-NR-095 DONE: workspace-kinds Req 8.1-8.13 (Command_Line_Position Top/Bottom Kind_Profile attribute; TOML round-trip with `#[serde(default)]`; applied in unsplit/split-region/detached render paths resolved per instance via `command_line_position_for`; Kinds Editor toggle; RESET BARE -> Top; the ISPF `COMMAND` / `COMMAND TOP` / `COMMAND BOTTOM` runtime command sharing one `set_active_command_line_position` seam with the editor and persisting to the Kind file via `persist_kind_config`; `COMMAND` disjoint from `COMMANDS`). Impl: `CommandLinePosition` enum + `KindProfile.command_line_position`; position-aware `split_region_strip_rects`; COMMAND dispatch arm. ~10 new tests (2 workspace_kind unit + split Top/Bottom + 5 COMMAND + RESET-BARE + full-shell geometry); verify.ps1 CLEAN FULL nextest (9311 passed, 0 failed). Behaviour-additive, default Top. Builds on CR-NR-090 + CR-CH-041 + CR-NR-094 + B072. |

## Phase (scroll-amount-updown) -- CR-NR-087 (no-argument UP/DOWN honour the active SCROLL amount)

> Bare `UP`/`DOWN` (and PF7/PF8) scroll by the active `SCROLL ===>` amount
> (PAGE/HALF/MAX/CSR/DATA/n) instead of always one page. Completes a B046
> remnant (MAX-then-UP -> top). An explicit `UP n`/`DOWN n` still overrides.
> PF7/PF8 stay bound to UP/DOWN (no key re-binding). Default PAGE preserves
