...
| 2.2c | Open any menu (POM/Settings); shrink the window until a long option description wraps to a second line. | The wrapped continuation of the description hangs-indents under the START of the description column (under the first description character), NOT under the key column at the row's left edge. First-line column alignment (key \| command \| description) is unaffected. | B065 (OPEN -- description does not hang-indent on wrap; whole three-column row is one wrapping LayoutJob); menu-workspace 2.1a (wrap-alignment criterion TBD) | [F] |
...
| B065 (menu option description does not hang-indent when it wraps; continuation aligns under key column) | OPEN | 2.2c |
