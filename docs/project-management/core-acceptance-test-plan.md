...
| 7.3a | Set the `SCROLL ===>` amount to MAX (type `SCROLL MAX`, or `M`/`MAX` where the field accepts it), then press UP / DOWN (or F7/F8). | Scrolls to the top / bottom of the document (the active SCROLL amount governs bare UP/DOWN, ISPF-style); with SCROLL PAGE it is one page, HALF a half page, n = n lines. MAX is a SCROLL-field amount, not an F-key binding. | CR-NR-087 (DONE -- SCROLL-amount-aware UP/DOWN; AUTOMATED: nav_manager `up_by_amount`/`down_by_amount` + `resolve_scroll_action`, shell `command_up_with_scroll_max_scrolls_to_top`); navigation-commands Req 3.17-3.23; B046 | [ ] |
...
| CR-NR-087 (SCROLL-amount-aware bare UP/DOWN, B046 MAX modifier) | DONE -- retest 7.3a | 7.3a |
